  ...
  module ManifestMatchers
    class CreateGeneric
      ...
      def precedes?(first, second)
        before_refs = Array[first[:before]].flatten.map { |ref|
          ref.respond_to?(:to_ref) ? ref.to_ref : ref
        }
        require_refs = Array[second[:require]].flatten.map { |ref|
          ref.respond_to?(:to_ref) ? ref.to_ref : ref
        }
        return before_refs.include?(second.to_ref) || require_refs.include?(first.to_ref)
      end

      def notifies?(first, second)
        before_refs = Array[first[:notify]].flatten.map { |ref|
          ref.respond_to?(:to_ref) ? ref.to_ref : ref
        }
        require_refs = Array[second[:subscribe]].flatten.map { |ref|
          ref.respond_to?(:to_ref) ? ref.to_ref : ref
        }
        return before_refs.include?(second.to_ref) || require_refs.include?(first.to_ref)
      end
